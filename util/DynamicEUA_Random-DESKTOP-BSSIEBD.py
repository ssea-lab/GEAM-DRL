...
def random_allocate(servers, users, user_masks, user_num_per_sec, total_sec=100):
    ...
    for current_time in range(total_sec):
        ...
        for user_i in range(current_user_len):
            ...
            workload = user[2: -1]
            # 在可用server中随机找一个分配
            random_servers = user_within_servers[i][:]
            ...
            for server_id in random_servers:
                ...
                if can_allocate(workload, capacity):
                    ...
                    user_allocate_list[user_i] = server_id
                    server_allocate_num[server_id] += 1
                    # 记录该用户退出的时间，放到优先队列里
                    exit_time = current_time + user[-1]
                    exit_queue.push((exit_time, i, server_id))
                    break
